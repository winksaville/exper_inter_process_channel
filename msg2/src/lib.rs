...
impl Msg2 {
    ...
    pub fn from_serde_json_str(s: &str) -> Result<Self, Box<dyn Error>> {
        if MsgHeader::cmp_str_id_and_serde_json_msg_header(MSG2_ID_STR, s) {
            match serde_json::from_str::<Self>(s) {
                Ok(msg) => Ok(msg),
                Err(why) => Err(format!("Msg2::from_serde_json_str: {why}").into()),
            }
        } else {
            Err(
                format!("Msg2::from_serde_json_str: wrong id in {s}, expecting {MSG2_ID_STR}")
                    .into(),
            )
        }
    }
    ...
    pub fn from_serde_json_buf(buf: &[u8]) -> Result<Self, Box<dyn Error>> {
        if let Ok(s) = from_utf8(buf) {
            ...
            Err("Msg2::from_serde_json_buf: Not UTF8".into())
        }
    }
}
