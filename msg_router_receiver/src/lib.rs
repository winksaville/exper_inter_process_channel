...
unsafe impl Send for MsgRouterReceiver {}

// TODO: This Sync guarantee is valid because multiple threads will never access an Actor. ??
unsafe impl Sync for MsgRouterReceiver {}

impl Actor for MsgRouterReceiver {
...
impl Debug for MsgRouterReceiver {
...
const MSG_ROUTER_DISPATCHER_ACTOR_ID: AnId = anid!("91119a6d-e9be-4a38-b3c5-23d3249e8da7");
const MSG_ROUTER_DISPATCHER_PROTOCOL_SET_ID: AnId = anid!("a7ce89ee-a584-41a4-b848-a4246f7a91e6");

impl MsgRouterReceiver {
    pub fn new(name: &str, addr: &str) -> Self {
        // Create the msg_router ProtocolSet, `ps`.
        println!("MsgRouterReceiver::new({})", name);
        ...
        let msg_router_ps =
            ProtocolSet::new("msg_router_receiver_ps", MSG_ROUTER_DISPATCHER_PROTOCOL_SET_ID, pm);
...
mod test {
    ...
    fn test_1() {
        ...
        // Add MsgRouterReceiver to ActorExecutor
        let mrd1 = Box::new(MsgRouterReceiver::new("mrd1", mrd1_addr));
        ...
        // Send to MsgRouterReceiver thread which will deserialize the msg and
        ...
        // Read EchoRsp that MsgRouterReceiver sends to us the supervisor
        let msg_any = supervisor_chnl.receiver.recv().unwrap();
